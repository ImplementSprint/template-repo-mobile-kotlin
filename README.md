...
0. Repository-owned gate: `:app:assembleDebug` + `:app:testDebugUnitTest` on every push/PR
...
For `push` and `pull_request`, the caller workflow enforces build-and-unit-test validation before running the central orchestrator.
